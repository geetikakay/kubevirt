...
var _ = Describe(SIG("GuestAgent probe pause", decorators.GuestAgentProbes, Serial, func() {
	It("[test_id:TS-CNV82132-003] should resume probes and restart the VMI after annotation removal", func() {
	...
	Context("readiness probe with annotation-paused probes", func() {
		...
		It("[test_id:TS-CNV82132-004] should remain Ready with paused readiness probes", func() {
			vmi := libvmifact.NewFedora(
				libnet.WithMasqueradeNetworking(),
		...
		It("[test_id:TS-CNV82132-004b] should lose Ready when agent stops without pause annotation", func() {
			vmi := libvmifact.NewFedora(
				libnet.WithMasqueradeNetworking(),
				withReadinessProbe(createGuestAgentPingProbe(period, initialSeconds)),
			)
	...
	DescribeTable("annotation pause value handling",
		...
		Entry("[test_id:TS-CNV82132-011a] should pause probes when annotation is true", "true", true),
		Entry("[test_id:TS-CNV82132-011b] should resume probes when annotation is false", "false", false),
		Entry("[test_id:TS-CNV82132-011c] should resume probes when annotation is invalid", "invalid", false),
	)

	It("[test_id:TS-CNV82132-013] should remain paused across reconciliation cycles", func() {
		vmi := libvmifact.NewFedora(
			libnet.WithMasqueradeNetworking(),
			withLivenessProbe(createGuestAgentPingLivenessProbeWithThreshold(3)),
		)
