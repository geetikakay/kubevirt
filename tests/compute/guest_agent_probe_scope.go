...
var _ = Describe(SIG("GuestAgent probe pause scope", decorators.GuestAgentProbes, Serial, func() {
	...
	It("[test_id:TS-CNV82132-005] should not affect exec probe with pause annotation set", func() {
	...
	It("[test_id:TS-CNV82132-009] should suppress GuestAgentPingFailed events during pause", func() {
		vmi := libvmifact.NewFedora(
			libnet.WithMasqueradeNetworking(),
	...
	It("[test_id:TS-CNV82132-009b] should emit GuestAgentPingFailed events when pause annotation is not set", func() {
		vmi := libvmifact.NewFedora(
			libnet.WithMasqueradeNetworking(),
			withLivenessProbe(createGuestAgentPingLivenessProbeWithThreshold(20)),
		)
	...
	It("[test_id:TS-CNV82132-010] should allow SSH key injection with pause annotation set", func() {
	...
	It("[test_id:TS-CNV82132-014] should complete snapshot independently of probe pause", func() {
	...
	It("[test_id:TS-CNV82132-015] should persist through VM pause and unpause cycle", func() {
		vmi := libvmifact.NewFedora(
			libnet.WithMasqueradeNetworking(),
			withLivenessProbe(createGuestAgentPingProbe(probePeriod, probeInitialSeconds)),
		)
